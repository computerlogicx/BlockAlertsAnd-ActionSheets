...
+ (void)showInfoAlertWithTitle:(NSString *)title message:(NSString *)message
{
    ...
    [alert setCancelButtonWithTitle:NSLocalizedString(@"Dismiss", nil) block:NULL];
    [alert show];
    [alert release];
}
...
+ (void)showErrorAlert:(NSError *)error
{
    ...
    [alert setCancelButtonWithTitle:@"Dismiss" block:NULL];
    [alert show];
    [alert release];
}
    ...
    for (NSUInteger i = 0; i < _blocks.count; i++)
    {
        ...
        else if (i + 1 < _blocks.count)
        {
            ...
            CGSize size = [title sizeWithFont:buttonFont
                                  minFontSize:10
                               actualFontSize:nil forWidth:_view.bounds.size.width - kBorder * 2
